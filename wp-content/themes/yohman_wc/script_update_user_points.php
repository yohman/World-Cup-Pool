...
foreach ($blogusers as $user) 
{
	...
	echo '</tr>';
	// update user table
	add_user_meta( $user->ID, 'wc_points', $user_score);
	update_user_meta( $user->ID, 'wc_points', $user_score);

	$key = 'wc_points';
	$single = true;
	$user_scor = get_user_meta( $user->ID, $key, $single ); 
	echo '<p>The '. $key . ' value for user id ' . $user->ID . ' is: ' . $user_scor . '</p>'; 
