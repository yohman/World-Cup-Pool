...
<script>
	...
	yohman.sortUserTable = function(){
		...
		rows.sort(function(a, b) {
			...
			var A = Number(jQuery(a).children('td').eq(1).text().toUpperCase());
			var B = Number(jQuery(b).children('td').eq(1).text().toUpperCase());
			if(A < B) {
				return -1;
			}
		...
		jQuery.each(rows,function(i,val){
			...
			if(i>0)
			{
				if(thispoints != prev)
				{
					...
					realcounter++;
					counter = realcounter;
					jQuery(val).children('td').eq(0).html(realcounter);
				}
				else
				{
					realcounter++;
...
function wc_prediction_post_type() {
	...
	$labels = array(
		...
		'parent_item_colon'  => '',
		'menu_name'          => 'World Cup predictions'
	);
	$args = array(
		'labels'        => $labels,
...
/****************************************
 *										*
 * 	Adds wc_rankings_Widget widget.		*
 *										*
 ***************************************/

class wc_rankings_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'wc_rankings_widget', // Base ID
			__('World Cup Rankings', 'text_domain'), // Name
			array( 'description' => __( 'World Cup Rankings Widget', 'text_domain' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		include 'wc_userrankings_include_widget.php';
	...
	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Leader Board', 'text_domain' );
		}
		?>
		<p>
...
} // class wc_rankings_Widget

// register wc_rankings_Widget widget
function register_wc_rankings_widget() {
    register_widget( 'wc_rankings_Widget' );
}
add_action( 'widgets_init', 'register_wc_rankings_widget' );


/****************************************
 *										*
 * 	Adds wc_nextmatches_Widget widget.		*
 *										*
 ***************************************/

class wc_nextmatches_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'wc_nextmatches_widget', // Base ID
			__('World Cup Next Matches', 'text_domain'), // Name
			array( 'description' => __( 'World Cup Next Matches Widget', 'text_domain' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		include 'wc_next_matches_include_widget.php';
	...
	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Next Matches', 'text_domain' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:' ); ?></label> 
...
} // class wc_nextmatches_Widget

// register wc_nextmatches_Widget widget
function register_wc_nextmatches_widget() {
    register_widget( 'wc_nextmatches_Widget' );
}
add_action( 'widgets_init', 'register_wc_nextmatches_widget' );
